...
namespace Wx.Common.Utils
{
    ...
    {
        /// <summary>
        /// 【zdwen 2014-8-26-152817】对列表对象进行计数。
        /// </summary>
        ...
        /// <summary>
        /// 【zdwen 2014-8-27-174439】计算一个对象所占用内存大小。
        /// </summary>
        ...
        {
            ///【zdwen 2012-2-18-233447】这里实在是没有办法在声明泛型T的时候约束他一定要继承自System.Enum，至少在.NET 4.0里面都还是不行的，不只是CRC遇到这个问题，
            ...
            throw new Exception("【zdwen 2014-1-26-115324】运行时异常，输入泛型类型<T>类型应该是一个枚举类型。");
        }
        ...
        public static byte[] Compress(Encoding encoding, string strSrc)
        {
            ...
            using (MemoryStream ms = new MemoryStream())
            {
                ...
                ///【zdwen 2013-12-25-113331】在ToArray的时候zipStream必须依据是关闭状态，也就是说，不能强迫症式的两个using连写的方式！！
